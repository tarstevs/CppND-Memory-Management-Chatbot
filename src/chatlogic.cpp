...
/**
 *
 * `LoadAnswerGraphFromFile`sets up the entire network and connects nodes with edges
 */
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
  ...
  /**
   *
 * 1. Create local ChatBot instance on the stack
 * 2. Use move semantics to pass the ChatBot instance into the root node. Make sure that
 * ChatLogic has no ownership relation to the ChatBot instance and thus is no longer
 * responsible for memory allocation and deallocation. Note that the member _chatBot of
 * ChatLogic remains so it can be used as a communication handle between GUI and ChatBot instance.
 ...
 * is called should be printed to the console. When sending a query to the ChatBot, the output
